...
const MoreInfo = () => {
    ...
    const handleMajorChange = newValue => { 
      ...
      setMajor(index_of_major >= 0 ? major.filter(maj => newValue !== maj) : [...major, newValue]);
    }

    const handleMinorChange = newValue => {
      const index_of_minor = minor.indexOf(newValue)
      setMinor(index_of_minor >= 0 ? minor.filter(maj => newValue !== maj) : [...minor, newValue]);
    }
      ...
      <div className = "pink_shape">
        ...
        <fieldset className ="totalform">
          ...
          <div className="dd-wrapper">
            <div className="dd-header" onClick={toggleMajor}>
                    <div className="dd-header-title">Majors
                      <i className = {majorArrow}/>
                    </div>
            </div>
            {isMajorOpen && <ul className="dd-list">
              {majors.map(item => (
                  <li className="dd-list-item" key={item.name}>
                    <DropDownItem name={item.name} setInfo = {handleMajorChange}
                    selectedItems = {major}/>
                  </li>
                ))
              }
          ...
          <div className="dd-wrapper">
            <div className="dd-header" onClick={toggleMinor}>
                    <div className="dd-header-title">Minors
                      <i className = {minorArrow}/>
                    </div>
            </div>
            {isMinorOpen && <ul className="dd-list">
              {minors.map(item => (
                  <li className="dd-list-item" key={item.name}>
                    <DropDownItem name={item.name} setInfo = {handleMinorChange}
                    selectedItems = {minor}/>
                  </li>
                ))
              }
          ...
          <div className="dd-wrapper">
            <div className="dd-header" onClick={toggleCollege}>
                    <div className="dd-header-title">{college === "" ? "College" : college}
                      <i className = {collegeArrow}/>
                    </div>
            </div>
            {isCollegeOpen && <ul className="dd-list">
              {colleges.map(item => (
                  <li className="dd-list-item" key={item}>
                    <DropDownItem name={item} setInfo = {handleCollegeChange}
